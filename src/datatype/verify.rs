        ...
        for sig in &signed.signatures {
            match self.keys.get(&sig.keyid) {
                Some(key) => {
                    ...
                    let sig = match sig.sig.from_base64() {
                        Ok(sig)  => sig,
                        Err(err) => {
                            trace!("couldn't convert sig from base64 for {}: {}", public, err);
                            continue;
                        }
                    };

                    let sigtype: SigType = key.keytype.clone().into();
                    sigtype.verify(&canonical, pem, &sig)
                        .map(|_| {
...
mod tests {
    ...
    fn test_rsa_verify() {
        ...
        let der = pem::parse(RSA_2048_PUB).unwrap().contents;
        SigType::RsaSsaPss.verify(msg, &der, &sig_raw).expect("couldn't verify message");

        {
            let mut bad_msg = msg.clone();
            bad_msg[0] = msg[0] ^ 0x01;
            if SigType::RsaSsaPss.verify(&bad_msg, RSA_2048_PUB.as_bytes(), &sig_raw).is_ok() {
        ...
        }
        {
            ...
            if SigType::RsaSsaPss.verify(msg, RSA_2048_PUB.as_bytes(), &bad_sig).is_ok() {
                panic!("Expected signature verification to fail");
            };
        }
    }
